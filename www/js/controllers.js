angular.module('starter.controllers', ['ui.router', 'ngCordova'])
  .controller('mainCtrl', function($scope, $state, $cordovaFile, $ionicPlatform, $http, $filter, $cordovaCamera, $ionicPopup) {
    $scope.data={
      loc:{
        lat: 39.5784168,
        lng: -104.8561335
    ...
    $scope.getFeedbackCode = function(){
      $ionicPopup.prompt({
        title: 'Feedback Code',
        okText: 'Submit'
      }).then(function(res){
        var queryString = "feedback="+res;
        $http.post("https://sales.jabtools.com/ajax/mobile_v012.php",queryString, {headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(function(resp){
          // $scope.parseResp(resp);
          $state.go('tower', {lat: $scope.data.lat, long: $scope.data.lng, feedback: resp.data});
        }, $scope.onFail);
      });
    };

    // Login stuff, remember to add $ionicModal to deps
