...
.controller('infoCtrl', function($scope, $state, $cordovaFile, $filter, $http) {
    ...
    $scope.enterInfo = function(){
        var queryString = encodeURI("record="+$scope.uuid+"&first_name="+$scope.data.cust.fname+"&last_name="+$scope.data.cust.lname+"&phone_work="+$scope.data.cust.phone+"&api_email_entry_c="+$scope.data.cust.email);
        alert(queryString);
